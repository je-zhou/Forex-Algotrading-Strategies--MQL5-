...
int OnInit() {
   handleFastMa = iMA(_Symbol, Timeframe, FastMA, 0, MODE_SMA, PRICE_CLOSE);
   handleSlowMa = iMA(_Symbol, Timeframe, SlowMA, 0, MODE_SMA, PRICE_CLOSE);
...
void TradeLogic() {
   ...
   //--- Buy Conditions: Fast MA crosses above Slow MA
   if (fastMa[0] <= slowMa[0] && fastMa[1] > slowMa[1]) {
      OnBuy();
   }
   
   //--- Sell Conditions: Fast MA crosses below Slow MA
   if (fastMa[0] >= slowMa[0] && fastMa[1] < slowMa[1]) {
      OnSell();
   }
}
